...
Description: Transactional email sending utilities (password reset, verification, invitations).
...
Impact: Without this module, password reset and invitation flows cannot notify the user.

Email backend priority:
  1. SMTP_HOST configured → uses stdlib smtplib (Mailpit in Docker dev)
...
async def _dispatch_email(email: str, subject: str, html: str, label: str, link: str) -> None:
    ...
    try:
        ...
        logger.error("❌ Error enviando email de %s a %s: %s", label, email, exc)
        _log_fallback(label, email, link)
...
def _log_fallback(label: str, email: str, link: str) -> None:
    logger.info(
        "\n%s\n"
        "📧 EMAIL DE %s (sin backend de email — copiar enlace del log)\n"
        "   Para: %s\n"
        "   Enlace: %s\n"
        "%s",
        "=" * 60,
