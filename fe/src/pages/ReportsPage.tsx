    ...
    <div className="mx-auto max-w-5xl space-y-6 p-6">
      ...
      <div className="rounded-xl border border-border bg-surface p-6 shadow-sm">
        <div className="mb-4 grid grid-cols-1 gap-4 sm:grid-cols-4">
          ...
          <div>
            <label className="mb-1 block text-sm font-medium text-text-secondary">Fecha inicio</label>
            <input
              type="date"
              value={startDate}
          ...
          <div>
            <label className="mb-1 block text-sm font-medium text-text-secondary">Fecha fin</label>
            <input
              type="date"
              value={endDate}
        ...
        {report && (
          <>
            <div className="mb-4 flex items-center justify-between border-b border-border pb-3">
              <h2 className="text-lg font-bold text-text-primary">{report.farm_name}</h2>
              <div className="flex gap-2">
                <button
                  onClick={() => handleDownload("pdf")}
                  className="flex items-center gap-1.5 rounded-lg border border-border px-3 py-1.5 text-sm hover:bg-surface-alt"
                >
                  <FileText size={15} />
                  PDF
                </button>
              ...
              <div className="mb-6">
                ...
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
                  <div className="rounded-lg bg-surface-alt p-3 text-center">
                    <p className="text-xs text-text-muted">Bovinos</p>
                    <p className="text-xl font-bold text-text-primary">{report.productive.total_bovines}</p>
                  </div>
                  <div className="rounded-lg bg-surface-alt p-3 text-center">
                    <p className="text-xs text-text-muted">Machos</p>
                    <p className="text-xl font-bold text-text-primary">{report.productive.males}</p>
                  </div>
                  <div className="rounded-lg bg-surface-alt p-3 text-center">
                    <p className="text-xs text-text-muted">Hembras</p>
                    <p className="text-xl font-bold text-text-primary">{report.productive.females}</p>
                  </div>
                  <div className="rounded-lg bg-surface-alt p-3 text-center">
                    <p className="text-xs text-text-muted">Peso Prom.</p>
                    <p className="text-xl font-bold text-text-primary">{report.productive.avg_weight ?? "-"}</p>
                  </div>
                  <div className="rounded-lg bg-surface-alt p-3 text-center">
                    <p className="text-xs text-text-muted">Leche (L)</p>
                    <p className="text-xl font-bold text-text-primary">{report.productive.total_milk_liters.toFixed(1)}</p>
                  </div>
                  <div className="rounded-lg bg-surface-alt p-3 text-center">
                    <p className="text-xs text-text-muted">Terneros</p>
                    <p className="text-xl font-bold text-text-primary">{report.productive.total_calves}</p>
                  </div>
                </div>
                {Object.keys(report.productive.calves_by_age_group).length > 0 && (
                  <div className="mt-3">
                    <h4 className="mb-1 text-sm font-semibold text-text-secondary">Terneros por edad</h4>
            ...
            {report.sanitary && (
              <div className="mb-6">
                ...
                <div className="grid grid-cols-3 gap-3 sm:grid-cols-3 lg:grid-cols-3">
                  <div className="rounded-lg bg-surface-alt p-3 text-center">
                    <p className="text-xs text-text-muted">Tratamientos</p>
                    <p className="text-xl font-bold text-text-primary">{report.sanitary.total_treatments}</p>
                  </div>
                  <div className="rounded-lg bg-surface-alt p-3 text-center">
                    <p className="text-xs text-text-muted">Planes Activos</p>
                    <p className="text-xl font-bold text-text-primary">{report.sanitary.active_sanitary_plans}</p>
                  </div>
                  <div className="rounded-lg bg-surface-alt p-3 text-center">
                    <p className="text-xs text-text-muted">Pendientes</p>
                    <p className="text-xl font-bold text-text-primary">{report.sanitary.pending_treatments}</p>
                  </div>
                </div>
                {Object.keys(report.sanitary.treatments_by_type).length > 0 && (
                  <div className="mt-3">
                    <h4 className="mb-1 text-sm font-semibold text-text-secondary">Por tipo</h4>
                    <div className="flex flex-wrap gap-2">
